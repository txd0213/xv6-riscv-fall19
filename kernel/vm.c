...
/* 
  map physical memory from one page to another 
  while still holding the same virtual address
*/
void copypages(pagetable_t from, pagetable_t to, uint64 vafrom, uint64 vato)
{
  pte_t *pfrom, *pto;
  // vafrom = PGROUNDDOWN(vafrom);
  // vato = PGROUNDUP(vato);
  // vafrom = PGROUNDUP(vafrom);
  for (uint64 i = vafrom; i < vato; i += PGSIZE)
  {
    pfrom = walk(from, i, 0);
    if (pfrom == 0)
      panic("mapping from error");
    pto = walk(to, i, 1);
    if (pto == 0)
      panic("mapping to error");
    *pto = (*pfrom) & (~PTE_U);
  }
}
